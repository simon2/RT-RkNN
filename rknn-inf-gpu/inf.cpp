...
// Helper function to convert Line to CudaLine
CudaLine convert_to_cuda_line(const Line& line) {
    CudaLine cuda_line;
    cuda_line.a = line.a;
    cuda_line.b = line.b;
    cuda_line.x_val = line.x_val;
    cuda_line.is_vertical = line.is_vertical;
    cuda_line.valid_side = line.valid_side;
    return cuda_line;
}

// Helper function to convert Point to CudaPoint
CudaPoint convert_to_cuda_point(const Point& point) {
    CudaPoint cuda_point;
    cuda_point.x = point.x;
    cuda_point.y = point.y;
    cuda_point.id = point.id;
    return cuda_point;
}

// Helper function to extract all user points from R*-tree into a vector
void extract_all_points_from_tree(shared_ptr<RStarNode> node, vector<CudaPoint>& points) {
    if (!node) return;

    if (node->is_leaf) {
        // For leaf nodes, add all points
        for (const auto& entry : node->entries) {
            points.push_back(convert_to_cuda_point(entry.point));
        }
    } else {
        // For internal nodes, recursively process children
        for (const auto& entry : node->entries) {
            extract_all_points_from_tree(entry.child, points);
        }
    }
}
...
int main( int argc, char* argv[] ) 
{
    ...
    try
    {
        ...
        if (is_cuda_available()) {
            cout << "Using GPU acceleration for verification" << endl;
            cuda_init();

            start_time = get_wall_time();

            // Convert bisectors to CUDA format
            vector<CudaLine> cuda_bisectors;
            for (const auto& bisector : bisectors) {
                cuda_bisectors.push_back(convert_to_cuda_line(bisector));
            }

            // Extract all user points from R*-tree
            vector<CudaPoint> cuda_users;
            extract_all_points_from_tree(usr_rtree.get_root(), cuda_users);

            // Perform GPU-based verification
            vector<CudaPoint> cuda_rknn_results;
            gpu_get_rknn_candidates(cuda_users, cuda_bisectors, cuda_rknn_results, k);

            end_time = get_wall_time();
            verification_time = end_time - start_time;

            cout << "Found " << cuda_rknn_results.size() << " RkNN results." << endl;
            cout << "Verification time (GPU): " << verification_time << "[s]." << endl << endl;

            // Clean up CUDA resources
            cuda_cleanup();
        } else {
            cout << "CUDA not available, falling back to CPU verification" << endl;
            start_time = get_wall_time();
            ...
            cout << "Verification time (CPU): " << verification_time << "[s]." << endl << endl;
        }
